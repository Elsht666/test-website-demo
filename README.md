...
3. 页面解析失败 → 探测站点 `/favicon.ico` → DuckDuckGo 图标服务（`icons.duckduckgo.com`，全部在 CF 服务端完成）
4. 返回 `{ url: 真实图标地址, preview: /api/icon 同域预览地址, source: 来源 }`；前端预览与"新标签页打开"走 `/api/icon` 转发，浏览器不直连被墙域名
